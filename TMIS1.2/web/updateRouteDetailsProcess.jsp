    ...
    <body>
      ...
      <c:set var="driverName" value="${param.driverName}" />
         ...
         <sql:update dataSource="${TruckMIS}" var="dbResult">
            UPDATE routes SET date = ?, driverName = ?, driverSurname = ?, truckNumberPlate = ?, truckVinNumber = ?, departureLocation = ?, destinationLocation = ?, routeApprovedBy = ? 
            WHERE id =?
      ...
      <c:if test="${dbResult>=1}">
         <c:redirect url="viewRouteBrief.jsp" >
            <c:param name="msg" value="Route Details Have Been Updated" />
         </c:redirect>
      </c:if>
