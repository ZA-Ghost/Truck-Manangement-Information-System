    ...
    <body>
        ...
        <c:set var="brand" value="${param.brand}" />
         ...
         <sql:update dataSource="${TruckMIS}" var="dbResult">
		  ...
		  INSERT INTO servicemanagement (brand, model, dateBought, vinNumber, numberPlate, licenseDisk, kilometers, dateServiced, diagnosis, cost)
		  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      <c:if test="${dbResult>=1}">
         <c:redirect url="viewServiceBrief.jsp" >
            <c:param name="msg" value="Service Details Have Been Uploaded" />
         </c:redirect>
      </c:if>
